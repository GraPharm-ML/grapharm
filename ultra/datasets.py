...
class TransductiveDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        ######################
        import json
        with open(os.path.join(os.path.dirname(train_files[0]), "inv_entity_vocab.json"), "w") as f:
            json.dump(test_results["inv_entity_vocab"], f, indent=4)
        
        ####################
        # Added by Hoa end #
        ####################
        with open(os.path.join(os.path.dirname(train_files[0]), "inv_rel_vocab.json"), "w") as f:
            json.dump(test_results["inv_rel_vocab"], f, indent=4)
